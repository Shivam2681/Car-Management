  ...
  {filteredCarList.map((car) => (
    ...
    >
      ...
      <CardContent className="p-5">
        ...
        <p className="text-sm text-muted-foreground mb-2">{car.modelName}</p>
  ...
  {car.description}
      ...
      <CardFooter className="p-4 pt-0 flex justify-between items-center">
        <Link href={`/dashboard/cars/${car.id}`}>
          <Button variant="link" className="p-0 text-primary font-medium transition-all duration-500 ease-in-out hover:underline">
            View Details
          </Button>
        </Link>
        <div className="flex gap-2">
      ...
      {filteredCarList.length === 0 && (
        <div className="text-center py-10">
          <p className="text-muted-foreground">No cars found matching the search query.</p>
        </div>
      )}
    </div>
  )
